...
graph_dropdown = dbc.Card(
    dbc.Container(
        html.Div(
            [
                dbc.Label("Choose a graph", html_for="dropdown"),
                dcc.Dropdown(
                    id="graph-dropdown",
                    options=[
...
country_dropdown = html.Div(
    [
        dbc.Label("Choose a Country from Asia", html_for="dropdown", id='country-options'),
        dcc.Dropdown(
            id="country-dropdown",
...
def update_option_text(continent):
    return dbc.Label("Choose a Country from {}".format(continent))
